...
export default function HomePage({ socket }) {
  ...
  var reloadCount =
    sessionStorage.getItem("reloadCount") === null
      ? 0
  ...
  useEffect(() => {
    if (reloadCount < 3 && Cookies.get("_auth") === undefined) {
    ...
    const getHistory = async () => {
      try {
        ...
        for (const history of historyData) {
          ...
          const date = new Date(history.time_started).toLocaleDateString(
            "en-US"
          );
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="flex flex-col items-center w-3/4">
            ...
            >
              <div className="font-medium text-2xl mb-7">
                {numQuestionsAttempted === 1
                  ? `1 Question attempted in ${new Date().getFullYear()}`
                  : `${numQuestionsAttempted} Questions attempted in ${new Date().getFullYear()}`}
              </div>
              <CalendarHeatmap
                ...
                tooltipDataAttrs={(value) => {
                  if (value.count === 1) {
                    return {
                      "data-tooltip-content": `1 question attempted on ${value.date}`,
                      "data-tooltip-id": "calendar-tooltip",
                    };
                  }
                  return {
                    "data-tooltip-content": `${value.count} questions attempted on ${value.date}`,
                    "data-tooltip-id": "calendar-tooltip",
                  };
                }}
              />
            ...
            >
              ...
              <PieChart
                series={[{ data: difficulties, innerRadius: 80 }]}
                {...size}
              >
                <PieCenterLabel>
                  {hasHistory ? "Difficulty" : "No attempts"}
                </PieCenterLabel>
              </PieChart>
            </Card>
          </div>
